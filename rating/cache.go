...
var (
	...
	SavadTracksCache    map[string][]spotify.SavedTrack    = make(map[string][]spotify.SavedTrack)
...
func GetTracksFromSaved(ctx context.Context, user wrapper.User, cache_key string) ([]spotify.SavedTrack, error) {
	...
	cached_playlist, ok := SavadTracksCache[cache_key]
	if !ok{
		fetched_playlist, err := FetchSavedTracks(ctx, user)
		if err != nil{
			return nil, err
		}
		SavadTracksCache[cache_key] = fetched_playlist
